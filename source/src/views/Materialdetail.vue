...
var duihaun = () => {
  ...
  router.push(
    "/submitintegral?id=" + route.query.id + "&type=" + route.query.type + "&yhid=" + route.query.yhid
  );
};

var chakandianpu = () => {
   HttpHelper.Post("coupon/judgmenttype", { id: route.query.yhid }).then(
      (res) => {
        console.log(res);
        if(res.bigcategory_id=="9"){
            router.push("/carwashplace");
        }else{
...
<template>
  <div v-if="page.Res != null">
    <div class="padding-15">
      <div class="bg-w border-radius-9">
        ...
        <div class="flex-row flex-center padding-15">
          ...
          <img
            ...
            class="icon-18"
          />
          <div class="margin-left-4 c-6 f-18 bold">
            {{ pointsmadetail.point }}
          </div>
        </div>
        ...
        <div v-if="pointsmadetail.type == 'A'">
          <div
            class="
              ...
              f-14
              bold
              margin-top-26 margin-left-14 margin-right-14
            "
            @click="chakandianpu"
          >
            查看可用门店
      ...
      <div
        class="f-14 c-2 bold margin-top-20"
        v-if="evaluatelist.length > 0 && pointsmadetail.type != 'A'"
      >
        产品评价
      </div>

      <div
        class="bg-w border-radius-9 margin-top-10 padding-15"
        style="padding: 18px 9px 0px"
        v-if="evaluatelist.length > 0 && pointsmadetail.type != 'A'"
      >
        ...
        <div class="center h-44 line-height-44 f-11" @click="chakanall">
          查看全部评论
        </div>
      </div>
      ...
      <div class="position-bottom" style="bottom: 0px">
        <div
          class="h-40 line-height-40 center f-16 c-w bold bg-5"
          @click="duihaun()"
        >
          立即兑换
        </div>
